describe('StadsBingo Tests', () => {
  ...
  test('1. Team login should validate team code', () => {
    ...
    expect(validateTeamCode('')).toEqual({ valid: false, error: 'Team code is required' })
    expect(validateTeamCode('TEAM123')).toEqual({ valid: true, teamId: 'team-123' })
    expect(validateTeamCode('INVALID')).toEqual({ valid: false, error: 'Invalid team code' })
  })
  ...
  test('3. Submission should require text or image', () => {
    ...
    expect(validateSubmission('', '')).toEqual({ valid: false, error: 'Text or image required' })
  ...
  test('8. Feedback is required when giving feedback status', () => {
    ...
    expect(validateFeedback('FEEDBACK', '')).toEqual({ valid: false, error: 'Feedback text is required' })
    expect(validateFeedback('FEEDBACK', 'Please improve this')).toEqual({ valid: true })
